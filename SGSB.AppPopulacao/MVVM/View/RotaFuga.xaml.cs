...
public partial class RotaFuga : ContentPage
{
    private List<RotaFugaModel> lstRotaFuga;
    public RotaFuga()
    {
        SetInitialPosition();
        //MakeApiRequest();
    ...
    private async Task LoadRotaFuga() 
    {
        lstRotaFuga = new List<RotaFugaModel>();
        lstRotaFuga = await RotaFugaService.GetRotaFuga();
   


        foreach (var item in lstRotaFuga.Where(x => string.IsNullOrEmpty(x.Latitude) && string.IsNullOrEmpty(x.Longitude)).ToList())
        {
    ...
    private async void SetInitialPosition()
    {
        try
        {
            ...
            if (location != null)
            {
               ...
               await DisplayDrivingRoute(location.Latitude, location.Longitude, -11.041726, -38.784730);
            }
        }
        catch (Exception ex)
        {
    ...
    private async void MakeApiRequest()
    {
        ...
        try
        {
            ...
            string apiUrl = "http://api.sgsb.com.br/API/ListarRotaFuga";
            ...
            using (HttpClient client = new HttpClient())
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    lstRotaFuga = await response.Content.ReadFromJsonAsync<List<RotaFugaModel>>();
